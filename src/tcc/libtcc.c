...
PUB_FUNC void *tcc_malloc(unsigned long size)
{
    void *ptr;
    ptr = malloc(size);
    // nk_vc_printf("malloc %lu returns %p\n", size, ptr);
    if (!ptr && size){
        tcc_error("memory full");
        nk_vc_printf("ERROR: memory full\n");
    }
#ifdef MEM_DEBUG
    mem_cur_size += malloc_usable_size(ptr);
...
PUB_FUNC void *tcc_realloc(void *ptr, unsigned long size)
{
    ...
    mem_cur_size -= malloc_usable_size(ptr);
#endif
    ptr1 = nk_tcc_realloc(ptr, size);

    //ptr1 = malloc(ptr, size); // edit to eliminate undefined ref
    if (!ptr1 && size)
        tcc_error("memory full");
#ifdef MEM_DEBUG
...
static int tcc_compile(TCCState *s1)
{
    ...
    if (setjmp(s1->error_jmp_buf) == 0) {
        ...
        if (tok != TOK_EOF)
            expect("declaration");
        if (pvtop != vtop)
            tcc_warning("internal compi`ler error: vstack leak? (%d)", vtop - pvtop);
...
LIBTCCAPI TCCState *tcc_new(void)
{
    ...
    if (!s){
        return NULL;
        nk_vc_printf("fail to malloc state\n");
    }
    tcc_state = s;
#ifdef _WIN32
...
// nk_vc_printf("\nsymtab->sh_name = %d, sh_num = %d, sh_type = %d, sh_size = %d, sh_entsize = %d, sh_addr = %p\n", s->symtab->sh_name, s->symtab->sh_num, s->symtab->sh_type, s->symtab->sh_size, s->symtab->sh_entsize, s->symtab->sh_addr);
...
#endif
nk_vc_printf("\nDone tcc_new\n");
    //nk_vc_printf("8 in tcc_new\n");
    return s;
    //nk_vc_printf("9 in tcc_new\n");
}
...
// nk_vc_printf("output type is %d", output_type);

    if (!s->nostdinc) {
        /* default include paths */
        /* -isystem paths have already been handled */
        tcc_add_sysinclude_path(s, CONFIG_TCC_SYSINCLUDEPATHS);
// nk_vc_printf("\nneed add sysinclude path = %s\n", CONFIG_TCC_SYSINCLUDEPATHS);
    }

    /* if bound checking, then add corresponding sections */
#ifdef CONFIG_TCC_BCHECK
nk_vc_printf("we have config tcc bcheck\n");
...
#ifdef TCC_TARGET_PE
    tcc_add_library_path(s, CONFIG_TCC_LIBPATHS);
    nk_vc_printf("we have TCC_TARGET_PE\n");
# ifdef _WIN32
    tcc_add_systemdir(s);
    nk_vc_printf("we have _WIN32\n");
# endif
#else
    /* add libc crt1/crti objects */
    if ((output_type == TCC_OUTPUT_EXE || output_type == TCC_OUTPUT_DLL) &&
        !s->nostdlib) {
    nk_vc_printf("we are adding crt1\n");
