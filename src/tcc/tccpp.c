...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if(!ts)
        nk_vc_printf("Fail to allocate for ts\n");
    table_ident[i] = ts;
    ts->tok = tok_ident++;
    ts->sym_define = NULL;
    ts->sym_label = NULL;
...
nk_vc_printf("%s redefined", get_tok_str(v, NULL));
    }

// nk_vc_printf("--------\nIn define_push v=%d, macro_type=%d\n--------\n", v, macro_type);

    s = sym_push2(&define_stack, v, macro_type, 0);
    s->d = str;
    s->next = first_arg;
    table_ident[v - TOK_IDENT]->sym_define = s;
// nk_vc_printf("--------\nIn define_push table_ident[v - TOK_IDENT]->sym_define = %p\n--------\n", s);
}
...
{
// nk_vc_printf("\nBegin define_find function\n--------\ntable_ident = %p\nv = %d\nTOK_IDENT = %d\ntok_indent = %d\n--------\n", table_ident, v, TOK_IDENT, tok_ident);

    v -= TOK_IDENT;

    if ((unsigned)v >= (unsigned)(tok_ident - TOK_IDENT)){
                            // nk_vc_printf("..99..Done define_find function");

        return NULL;
    }
                    // nk_vc_printf("..100..Done define_find function  ");
                    // nk_vc_printf("**%d**\n",v);
//nk_vc_printf("\n size of table_ident = %u\n", sizeof(table_ident));
// nk_vc_printf("table_ident[%d] = %p\n", 0, table_ident[0]);
// nk_vc_printf("table_ident[%d] = %p\n", v, table_ident[v]);
// nk_vc_printf("table_ident[%d]->sym_define = %p\n", 0, table_ident[0]->sym_define);
// nk_vc_printf("table_ident[%d]->sym_define = %p\n", v, table_ident[v]->sym_define);
    ...
    if (ch == '.' ||
        ...
        if (b != 10) {
            ...
            *q = '\0';
            ...
            //******xuan
            int temp = exp_val - frac_bits;
            int temp2 = 1;
            while(temp--)
                temp2*=2;
            d = d * temp2;
            //******xuan
            // d = tcc_ldexp(d, exp_val - frac_bits);
            t = toup(ch);
            if (t == 'F') {
                ch = *p++;
                tok = TOK_CFLOAT;
                           ...
                           Sym **nested_list, Sym *s, struct macro_level **can_read_stream)
{
    ...
    if (tok == TOK___LINE__) {
        ...
        if (tok == TOK___DATE__) {
            ...
            printf("System date Not Supported!\n");
        } else {
            // snprintf(buf, sizeof(buf), "%02d:%02d:%02d",
            //          tm->tm_hour, tm->tm_min, tm->tm_sec);
            printf("System time Not Supported!\n");
        }
        cstrval = buf;
    add_cstr:
        t1 = TOK_STR;
...
ST_FUNC void next(void)
{
 ...
 redo:
    ...
    if (tok == TOK_PPNUM &&
        ...
        // nk_vc_printf("..100.. Done next() function\n");
